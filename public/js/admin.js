...
async function loadProperties() {
    ...
    const status   = document.getElementById('filterStatusSelect') ? document.getElementById('filterStatusSelect').value : '';
    ...
    props.forEach(p => {
        ...
        const revertBtn = p.status === 'Terjual'
            ? `<button class="btn-revert" onclick="revertToTersedia('${p._id}')" title="Ubah ke Tersedia"><i class="fa-solid fa-rotate-left"></i></button>`
            : '';
...
async function revertToTersedia(id) {
    if (!confirm('Ubah status properti ini dari Terjual → Tersedia?')) return;
    const res = await fetch(`/api/properties/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status: 'Tersedia', sold_at: null, sold_price: 0 })
    });
    if (res.ok) {
        loadProperties();
        loadStats();
